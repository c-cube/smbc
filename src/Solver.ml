...
module Make(Config : CONFIG)(Dummy : sig end) = struct
  ...
  let stat_num_memoized = ref 0
  let stat_num_memo_hit = ref 0
  ...
  (* a call to a memoized function.
     Invariants:
     [forall r in mc_registers, r < mc_offset] *)
  and memo_call = {
    mc_tbl: memo_tbl;
    ...
    mc_registers: memo_register_content registers; (* register -> concrete term *)
    mc_explanation: explanation; (* explanations so far (when evaluating arguments) *)
  }

  and memo_register_content =
    | Memo_reg_symb of memo_symbolic_value
    | Memo_reg_concrete of term
  ...
  and memo_decision_tree_cell =
    ...
    | Memo_single of int * memo_decision_tree_decide
    | Memo_multi of memo_decision_tree_decide IntMap.t (* parallel choice *)

  and memo_decision_tree_decide =
    | Memo_match of (register list * memo_decision_tree) ID.Map.t
    | Memo_if of memo_decision_tree * memo_decision_tree (* if *)
  ...
  let hash_reg (i,ty) = Hash.combine3 12345 i (Ty.hash ty)
  ...
  let eq_memo_sym a b = match a, b with
    | Memo_true, Memo_true
    | Memo_false, Memo_false -> true
    | Memo_cstor (c1,_,l1), Memo_cstor (c2,_,l2) ->
      Typed_cst.equal c1 c2 && CCList.equal eq_reg l1 l2
    | Memo_dom_elt c1, Memo_dom_elt c2 -> Typed_cst.equal c1 c2
    | Memo_true, _
    | Memo_false, _
    | Memo_cstor _, _
    | Memo_dom_elt _, _ -> false

  let hash_memo_sym = function
    | Memo_true -> 1
    | Memo_false -> 2
    | Memo_cstor (c,_,l) ->
  ...
  module Term = struct
    ...
    let pp_mc_dt_decide out = function
      | Memo_if (_,_) -> CCFormat.string out "if"
      | Memo_match _ -> CCFormat.string out "match"
    ...
    let pp_mc_dt out d = match d.mc_dt_cell with
      ...
      | Memo_single (i,m) ->
        Format.fprintf out "(@[%d %a@])" i pp_mc_dt_decide m
      | Memo_multi m ->
        Format.fprintf out "(@[%a@])"
          (IntMap.print ~start:"" ~stop:"" CCFormat.int pp_mc_dt_decide) m
    ...
    let pp_memo_reg pp_term out = function
  ...
  module Memo_utils : sig
    ...
    val subst : memo_register_content registers -> term -> term
  ...
  module Reduce = struct
    ...
    and compute_nf_memo_call (m:memo_call): explanation * term =
      let add_reg (m:memo_call)(i:int)(v:memo_register_content): memo_call =
        {m with mc_registers=IntMap.add i v m.mc_registers}
      in
      let add_reg_sym m i v = add_reg m i (Memo_reg_symb v) in
      let add_reg_concrete m i v = add_reg m i (Memo_reg_concrete v) in
      ...
      let as_value_noncached
          m r (t_concrete:term): memo_call * memo_symbolic_value option =
        ...
        let m =
          ...
          then m (* same old *)
            ...
            {m with mc_explanation=Explanation.append e_concrete m.mc_explanation; }
        in
                  ...
                  let (_,concrete_regs), sub_regs =
                    CCList.fold_map
                      (fun (n,c_regs) arg ->
                         ...
                         (n+1, IntMap.add n (Memo_reg_concrete arg) c_regs), reg)
                      (m.mc_dt.mc_dt_offset, m.mc_registers) args
                  in
                  let v = Memo_cstor (cstor, ty, sub_regs) in
                  let m = {m with mc_registers=concrete_regs;} in
                  m, Some v
                | None, Some (dom_elt,_) ->
      ...
      let as_value (m:memo_call) (r:int) : memo_call * memo_symbolic_value option =
        match Memo_utils.find_register m.mc_registers r with
          | Memo_reg_symb v -> m, Some v
          | Memo_reg_concrete t -> as_value_noncached m r t
      in
      ...
      let rec aux (m:memo_call): explanation * term =
        ...
        match m.mc_dt.mc_dt_cell with
          ...
          | Memo_single (i, decide) ->
            begin match as_value m i with
              | _, None -> Explanation.empty, Term.memo_call m
              | m, Some v -> aux_decide m decide v
            end
          | Memo_multi map -> aux_multi m map

      and aux_yield m =
        assert (m.mc_dt.mc_dt_cell = Memo_yield);
        incr stat_num_memo_hit;
        (* return the normal form, with concrete values replacing registers *)
        let lazy term = m.mc_dt.mc_dt_term in
        let nf_concrete = Memo_utils.subst m.mc_registers term in
        Log.debugf 5
          (fun k->k "(@[memo: yield %a@ expl: %a@])"
              Term.pp nf_concrete Explanation.pp m.mc_explanation);
        m.mc_explanation, nf_concrete

      (* follow the first branch of [m] whose register evaluates to a value,
         or block *)
      and aux_multi m map =
        let res =
          IntMap.to_seq map
          |> Sequence.find
            (fun (i, decide) -> match as_value m i with
               | _, None -> None
               | m, Some v -> Some (m, decide, v))
        in
        begin match res with
          ...
          | Some (m, decide, v) ->
            (* can decide on this register *)
            aux_decide m decide v
        end

      and aux_decide m (decide:memo_decision_tree_decide) v = match decide with
        | Memo_match map ->
          aux_match m v map
        | Memo_if (then_, else_) ->
          aux_if m v then_ else_

      and aux_if m v then_ else_ = match v with
        | Memo_true ->
          {m with mc_dt=then_} |> mk_memo_call |> aux
        | Memo_false ->
          {m with mc_dt=else_} |> mk_memo_call |> aux
        | _ -> assert false (* typing *)

      and aux_match m v dt_map = match v with
        | Memo_cstor (cstor, _, sub_regs) ->
          ...
          begin match ID.Map.get cstor.cst_id dt_map with
            ...
            | None ->
              errorf
                "decision tree on val %a@ does not have branch for cstor %a"
                Term.pp_sym_value v ID.pp cstor.cst_id
          end
        | _ -> assert false (* typing *)
        ...
        begin match nf_symb.term_deps with
          ...
          | l ->
            (* evaluation has not reached a value yet, because of
               ...
               We therefore expand the tree with a one- or multi-way decision
               on blocking registers *)
            let map =
              List.fold_left
                (fun map dep -> match dep with
                   | Dep_reg (r_idx,ty) ->
                     if IntMap.mem r_idx map then map
                     else begin match Ty.view ty with
                       | Arrow _ -> assert false
                       | Prop ->
                         let decide = aux_mk_if m r_idx nf_symb in
                         IntMap.add r_idx decide map
                       | Atomic (_, Data cstors) ->
                         let decide = aux_mk_match m r_idx cstors nf_symb in
                         IntMap.add r_idx decide map
                       | Atomic (_, Uninterpreted _uty) ->
                         assert false (* TODO *)
                     end
                   | dep ->
                     Log.debugf 1
                       (fun k->k
                           "unexpected concrete dep %a in symbolic computation"
                           pp_dep dep);
                     assert false)
                IntMap.empty l
            in
            assert (not (IntMap.is_empty map));
            let i, decide = IntMap.choose map in
            let new_dt =
              if IntMap.is_empty (IntMap.remove i map)
              then Memo_single (i, decide) (* singleton *)
              else Memo_multi map
            in
            set_cell m new_dt;
            aux m (* recurse *)
      ...
      (* decide on a datatype *)
      and aux_mk_match m r_idx cstors nf_symb: memo_decision_tree_decide =
        let map =
      ...
      (* decide on a boolean *)
      and aux_mk_if m r_idx nf_symb : memo_decision_tree_decide =
    ...
    and memo_call_init (f:memo_tbl) (args:term list): term =
      ...
      let regs =
        CCList.Idx.foldi
          (fun m i arg -> IntMap.add i (Memo_reg_concrete arg) m)
          IntMap.empty args
      in
      let mc = {
        mc_tbl=f;
    ...
    and mk_memo_call (m:memo_call): memo_call =
      ...
      let deref_deps (d:term_dep): term_dep list = match d with
        | Dep_reg (i,_) ->
          begin match Memo_utils.find_register m.mc_registers i with
            | Memo_reg_concrete t -> t.term_deps
            | Memo_reg_symb _ -> []
          end
        | _ -> [d]
      in
