  ...
  module Term = struct
    ...
    let pp_top ~ids out t =
      ...
      and pp_env out e =
        ...
        if !n > 0
        then Format.fprintf out "[@[<hv>%a@]]" (DB_env.pp pp) e'
      in
      pp out t
           ...
           and by_destruct =
             ...
             match Ty.view ty_ret with
               | Prop ->
                 ...
                 [Lazy.force test]
               | Atomic (_, Data cstors) ->
                 (* match, + recursion. We build the term
                    [(mu self. (fun x. match x with …)) the_param] *)
                 let rec test = lazy (
                   let parent = lazy (cst, Lazy.force test) in
                   (* type of the recursive function *)
                   let ty_self = Ty.arrow the_param.term_ty ty in
                   let m =
                     cstors
                     |> List.map
                       (fun (lazy cstor) ->
                          ...
                          (* in the context, we now have:
                             - the cstor arguments (n_ty_args)
                             - the function
                             - the `mu` binder *)
                          let db_ctx =
                            info.cst_db_ctx
                            |> DB_env.push ty_self (* mu *)
                            |> DB_env.push the_param.term_ty (* fun *)
                            |> DB_env.push_l ty_args
                          in
                          (* push sub-parameters: either sub-constants,
                             or applications of [self] if types match *)
                          let db_self = DB.make (n_ty_args+1) ty_self in
                          let sub_params =
                            List.mapi
                              (fun i ty_arg ->
                                 let db_arg = DB.make (n_ty_args-i-1) ty_arg in
                                 if Ty.equal ty_arg the_param.term_ty
                                 then (* call [self] *)
                                   Term.app (Term.db db_self) [Term.db db_arg]
                          ...
                          let can_use =
                            sub_params
                            @
                            (* account for the additional binders *)
                            List.map
                              (Term.shift_db (n_ty_args+2))
                              can_use
                          in
                          let rhs =
                            mk_sub_cst ty
                   ...
                   let rec_fun =
                     Term.mu
                       (Term.fun_ the_param.term_ty
                          (Term.match_
                             (Term.db (DB.make 0 the_param.term_ty))
                             m
                          ))
                   in
                   Term.app rec_fun [the_param]
                 ) in
                 [Lazy.force test]
               | Atomic (_, Uninterpreted)
               | Arrow _ -> []
      ...
      then t (* trivial *)
        ...
        let rec aux env t : term = match t.term_cell with
          ...
          | Const (c,env') ->
            ...
            let new_env = DB_env.map_some (aux env) env' in
      ...
      begin match info.cst_cases with
        | None ->
          ...
          then (
            ...
            Log.debugf 3
              (fun k->k
                  "(@[<hv1>@{<green>expand_cases@}@ @[%a@]@ \
                   :into (@[<hv>%a@])@ \
                   :ty %a@ :depth %d@ :ctx [@[%a@]]@ :can_use (@[<hv>%a@])@])"
                  Typed_cst.pp c (Utils.pp_list Term.pp) l
                  Ty.pp ty (Lazy.force info.cst_depth)
                  (DB_env.pp Ty.pp) info.cst_db_ctx
                  (Utils.pp_list Term.pp) info.cst_can_use);
