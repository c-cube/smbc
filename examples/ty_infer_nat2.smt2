...
; here, a,b,c,d are type constructors
...
(declare-datatypes
   ...
   ((ty
       ...
       (const (select_const_0 nat)))))
(define-funs-rec ((a () ty)) ((const z)))
(define-funs-rec ((b () ty)) ((const (s z))))
(define-funs-rec ((c () ty)) ((const (s (s z)))))
(define-funs-rec ((d () ty)) ((const (s (s (s z))))))
...
(assert-not
 (forall
    ...
    (not (ty_check e_nil e_2 
           (arrow (arrow c d) 
             (arrow (arrow b c) (arrow (arrow a b) (arrow a d))))))))(check-sat)
